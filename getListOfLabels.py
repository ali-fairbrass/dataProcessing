...
DIR = "C:\\Users\\ucfaalf\\Dropbox\\EngD\\Projects\\Chapter3 Classifier Evaluation\\goldenTestSet\\40LabelFiles\\Golden\\"

allLabels = os.listdir(DIR)

allLabelsList = []
for csvFile in allLabels:
    filePath = DIR + csvFile
    with open(filePath, 'rb') as f:
        reader = csv.reader(f)
        for row in reader:
            allLabelsList.append(row[1])

print sorted(list(set(allLabelsList)))